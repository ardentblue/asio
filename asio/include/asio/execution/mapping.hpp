...
struct mapping_t
{
  ...
  friend ASIO_CONSTEXPR mapping_t query(
    ...
    ASIO_NOEXCEPT_IF((is_nothrow_query<Executor, thread_t>::value))
  ...
  friend ASIO_CONSTEXPR mapping_t query(
      const Executor& ex, convertible_from_mapping_t,
      typename enable_if<
        !can_query<Executor, thread_t>::value
          && can_query<Executor, new_thread_t>::value
      >::type* = 0)
#if !defined(__clang__) // Clang crashes if noexcept is used here.
    ASIO_NOEXCEPT_IF((is_nothrow_query<Executor, new_thread_t>::value))
#endif // !defined(__clang__)
  {
  ...
  friend ASIO_CONSTEXPR mapping_t query(
      ...
      typename enable_if<
        !can_query<Executor, thread_t>::value
          && !can_query<Executor, new_thread_t>::value
          && can_query<Executor, other_t>::value
      >::type* = 0)
#if !defined(__clang__) // Clang crashes if noexcept is used here.
    ASIO_NOEXCEPT_IF((is_nothrow_query<Executor, other_t>::value))
#endif // !defined(__clang__)
  {
